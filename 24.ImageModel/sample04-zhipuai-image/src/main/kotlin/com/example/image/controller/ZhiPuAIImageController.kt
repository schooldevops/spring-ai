package com.example.image.controller

import org.springframework.ai.image.ImageModel
import org.springframework.ai.image.ImagePrompt
import org.springframework.web.bind.annotation.*

/** Sample 04: ZhiPuAI CogView */
@RestController
@RequestMapping("/api/image")
class ZhiPuAIImageController(private val imageModel: ImageModel) {

    data class ImageRequest(val prompt: String)

    @PostMapping("/generate")
    fun generateImage(@RequestBody request: ImageRequest): Map<String, Any> {
        val response = imageModel.call(ImagePrompt(request.prompt))
        return mapOf(
                "provider" to "ZhiPuAI",
                "model" to "CogView-3",
                "prompt" to request.prompt,
                "url" to response.result.output.url
        )
    }

    @GetMapping("/info")
    fun getInfo(): Map<String, Any> {
        return mapOf(
                "provider" to "ZhiPuAI",
                "model" to "CogView-3",
                "features" to listOf("Chinese AI provider", "High quality", "Multilingual")
        )
    }
}
